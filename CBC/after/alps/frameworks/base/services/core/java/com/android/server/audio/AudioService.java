...
public class AudioService extends IAudioService.Stub {
    ...
    /// M: Add for control debug log, only default enable it on eng load.
    protected static final boolean LOGD = "eng".equals(
                SystemProperties.get("ro.build.type", "user"));

    /** Debug audio mode */
    protected static final boolean DEBUG_MODE = Log.isLoggable(TAG + ".MOD", Log.DEBUG) || LOGD;

    /** Debug audio policy feature */
    protected static final boolean DEBUG_AP = Log.isLoggable(TAG + ".AP", Log.DEBUG) || LOGD;
    ...
    protected static final boolean DEBUG_DEVICES =
                                        Log.isLoggable(TAG + ".DEVICES", Log.DEBUG) || LOGD;
    ...
    /// M: listener record for audioprofile
    ...
    private static final boolean IS_DOLBY_DAP_SUPPORT = SystemProperties.getBoolean(
                                "ro.mtk_dolby_dap_support", false);
    /// @}
    ...
    public AudioService(Context context) {
        ...
        /// M: Add for the following setParameter and so on.
        ...
        //add for listen ACTION_AIRPLANE_MODE_CHANGED
    ...
    public void onMediaServerDied() {
        ...
        for (int streamType = numStreamTypes - 1; streamType >= 0; streamType--) {
            ...
            /// M: use (streamState.mIndexMin + 5) / 10 to replace 0 to avoid phone call volume
            /// index problem
            AudioSystem.initStreamVolume(streamType, (streamState.mIndexMin + 5) / 10,
    ...
    public void setVibrateSetting(int vibrateType, int vibrateSetting) {
        ...
        /// M: Notify vibrate setting change to AudioProfile to avoid CTS fail.
        notifyVibrateSettingChanged(vibrateType);
    }
    ...
    public void setMode(int mode, IBinder cb, String callingPackage) {
        if (DEBUG_MODE) { Log.v(TAG, "setMode(mode=" + mode + ", callingPackage="
              + callingPackage + ")"); }
    ...
    private void readAudioSettings(boolean userSwitch) {
        // restore ringer mode, ringer mode affected streams,
        //  mute affected streams and vibrate settings
        readPersistedSettings();
    ...
    public void setSpeakerphoneOn(boolean on){
        ...
        if (DEBUG_VOL) Log.d(TAG, "setSpeakerphoneOn " + mForcedUseForComm);
    }
    ...
    public void setBluetoothScoOn(boolean on){
        ...
        if (DEBUG_VOL) Log.d(TAG, "setBluetoothScoOn " + mForcedUseForComm);
    }

    /** @see AudioManager#isBluetoothScoOn() */
    public boolean isBluetoothScoOn() {
        if (DEBUG_VOL) Log.d(TAG, "isBluetoothScoOn " + mForcedUseForComm);
    ...
    public void startBluetoothSco(IBinder cb, int targetSdkVersion) {
        if (DEBUG_VOL) Log.d(TAG, "startBluetoothSco");
        int scoAudioMode =
    ...
    void startBluetoothScoInt(IBinder cb, int scoAudioMode){
        if (DEBUG_VOL) Log.d(TAG, "startBluetoothScoInt: scoAudioMode = " + scoAudioMode);
    ...
    public void stopBluetoothSco(IBinder cb){
        if (DEBUG_VOL) Log.d(TAG, "stopBluetoothSco");
    ...
    private class ScoClient implements IBinder.DeathRecipient {
            ...
            synchronized(mScoClients) {
                if (mStartcount != 0) {
                    try {
                        ...
                        Log.w(TAG, "clearCount() mStartcount: "+mStartcount+
                            " != 0 but not registered to binder");
                    }
                }
                mStartcount = 0;
                if (stopSco) {
        ...
        private void requestScoState(int state, int scoAudioMode) {
            if (DEBUG_VOL) {
                Log.d(TAG, "requestScoState:state = " + state + ", scoAudioMode =" + scoAudioMode);
            }
            checkScoAudioState();
            if (totalCount() == 0) {
    ...
    private BluetoothProfile.ServiceListener mBluetoothProfileServiceListener =
        ...
        public void onServiceConnected(int profile, BluetoothProfile proxy) {
            if (DEBUG_VOL) {
                Log.d(TAG, "BluetoothProfile.ServiceListener onServiceConnected = " + profile);
            }
        ...
        public void onServiceDisconnected(int profile) {
            if (DEBUG_VOL) {
                Log.d(TAG, "BluetoothProfile.ServiceListener onServiceDisconnected = " + profile);
            }
    ...
    public class VolumeStreamState {
        ...
        public void readSettings() {
            synchronized (VolumeStreamState.class) {
                ...
                for (int i = 0; remainingDevices != 0; i++) {
                    ...
                    // if no volume stored for current stream and device,
                    //  use default volume if default
        ...
        Log.d(TAG, "applyDeviceVolume" + "mStreamType:" + mStreamType + ",device:" + device);
        ...
        private String findDeviceUniqueId(int device) {
            ...
            switch (device) {
                ...
                case AudioSystem.DEVICE_OUT_BLUETOOTH_A2DP_SPEAKER:
                    uniqueId = mCurAudioRoutes.bluetoothName.toString();
                    break;
                ...
                case AudioSystem.DEVICE_OUT_USB_DEVICE:
                    DeviceListSpec deviceSpec = mConnectedDevices.get(device);
                    // TODO need test which one is best with mDeviceAddress and mName
                    uniqueId = deviceSpec == null ? "" : deviceSpec.mDeviceAddress;
                    break;
    ...
    /**
    *  M: add param shouldBroadcast to do not broadcast if headset is connected for ALPS00850163 @{
    */
    private void sendDeviceConnectionIntent(int device, int state, String address,
            String deviceName, boolean shouldBroadcast) {
        if (DEBUG_DEVICES) {
    ...
    private void onSetWiredDeviceConnectionState(int device, int state, String address,
        ...
        Log.d(TAG, "onSetWiredDeviceConnectionState:" + "device:" + device + ",state:" + state);
            ...
            if ((state == 0) && ((device == AudioSystem.DEVICE_OUT_WIRED_HEADSET) ||
                ...
                boolean isHeadsetOn = mConnectedDevices.containsKey(new StringBuilder().append("0x")
                        .append(AudioSystem.DEVICE_OUT_WIRED_HEADSET).append(":").toString());
    ...
    private class AudioServiceBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_DOCK_EVENT)) {
                ...
                if (DEBUG_VOL) {
                    Log.d(TAG, "BluetoothHeadset.ACTION_AUDIO_STATE_CHANGED mScoAudioState = "
                            + mScoAudioState);
                }
    ...
    */
    ...
    /**
     * M: Add for AudioProfile to adjust volume then ringermode not change.
     */
    public void setAudioProfileStreamVolume(int streamType, int index, int flags) {
        String caller = "AudioProfile";
        ensureValidStreamType(streamType);
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////
    // Register a RingerMode or volume listener callback to AudioService
    ///////////////////////////////////////////////////////////////////////////////////////////////

    /**
     * M: Register the IAudioProfileListener callback to AudioService to listen RingerMode
     * and volume changed.
     *
     ...
     * @hide
     * */
    public void listenRingerModeAndVolume(IAudioProfileListener callback, int event) {
        if (event != 0) {
            synchronized (mRecords) {
                // register callback in AudioProfileService
                //if the callback is exist, just replace the event.
                Record record = null;
                find_and_add: {
                ...
                if (event == AudioProfileListener.LISTEN_RINGERMODE_CHANGED) {
                    try {
                        record.mCallback.onRingerModeChanged(mRingerMode);
                    } catch (RemoteException e) {
                        remove(record.mBinder);
                        Log.e(TAG, "Dead object in listenRingerModeAndVolume,"
                                +" remove listener's callback!" + e);
                    }
                } else if (event == AudioProfileListener.LISTEN_RINGER_VOLUME_CHANGED) {
                    try {
                        record.mCallback.onRingerVolumeChanged(getStreamVolume(
                               AudioManager.STREAM_RING),
                             getStreamVolume(AudioManager.STREAM_RING), null);
                    } catch (RemoteException e) {
                        remove(record.mBinder);
                        Log.e(TAG, "Dead object in listenRingerModeAndVolume,"
                                +" remove listener's callback!" + e);
                    }
                } else if (event == AudioProfileListener.LISTEN_VIBRATE_SETTING_CHANGED) {
                    try {
                        record.mCallback.onVibrateSettingChanged(
                                    AudioManager.VIBRATE_TYPE_NOTIFICATION,
                ...
                Log.d(TAG, "listenRingerModeAndVolume with event = " + event
                        + " sucessed, mRecords.size() = "  + mRecords.size()
                        + ", record.mBinder = " + record.mBinder);
            }
        } else {
            remove(callback.asBinder());
            Log.d(TAG,
              "listenRingerModeAndVolume with LISTEN_NONE, so remove this listener's callback!");
        }
    }
    /**
     * M: Remove listener from record
     *
     * @param binder Listener binder
     */
        ...
        synchronized (mRecords) {
            ...
            while (iterator.hasNext()) {
                ...
                if (record.mBinder.equals(binder)) {
                    iterator.remove();
                    Log.d(TAG, "removed ringer mode or ringer volume change listener for: event = "
                            + record.mEvent +  "record.mBinder = " + record.mBinder
                            + ", client size = " + mRecords.size());
                }
            }
        }
    }
    ...
    /**
     * M: Notify the change of the RingerMode.
     */
    private void notifyRingerModeChanged() {
        if (!mRecords.isEmpty()) {
            Log.d(TAG, "notifyRingerModeChanged: New ringermode = " + mRingerMode
                    + ", clients = " + mRecords.size());
            synchronized (mRecords) {
                Iterator<Record> iterator = mRecords.iterator();
                while (iterator.hasNext()) {
                    Record record = (Record) iterator.next();
                    if (record.mEvent == AudioProfileListener.LISTEN_RINGERMODE_CHANGED) {
                        try {
                            record.mCallback.onRingerModeChanged(mRingerMode);
                        } catch (RemoteException e) {
                            iterator.remove();
                            Log.e(TAG, "Dead object in notifyRingerModeChanged,"
            ...
            Log.w(TAG, "notifyRingerModeChanged falled, because there are no listener!");
        }

    }

    /**
     * M: Notify the change of the ringer volume.
     */
    private void notifyRingerVolumeChanged(int oldVolume, int newVolume, String extra) {
        if (!mRecords.isEmpty()) {
            Log.d(TAG, "notifyRingerVolumeChanged: oldVolume = " + oldVolume
                    + ", newVolume = " + newVolume + ", clients = " + mRecords.size());
            synchronized (mRecords) {
                Iterator<Record> iterator = mRecords.iterator();
                while (iterator.hasNext()) {
                    Record record = (Record) iterator.next();
                    if (record.mEvent == AudioProfileListener.LISTEN_RINGER_VOLUME_CHANGED) {
                        try {
                            record.mCallback.onRingerVolumeChanged(oldVolume, newVolume, extra);
                        } catch (RemoteException e) {
                            iterator.remove();
                            Log.e(TAG, "Dead object in notifyRingerVolumeChanged,"
                                    + " remove listener's callback: record.mBinder = "
                                    + record.mBinder + ", clients = " + mRecords.size()
                                    + ", exception = " + e);
                        }
    ...
    /**
     * M: Notify the change of the vibrate setting.
     */
    private void notifyVibrateSettingChanged(int vibrateType) {
        if (!mRecords.isEmpty()) {
            synchronized (mRecords) {
                Iterator<Record> iterator = mRecords.iterator();
                while (iterator.hasNext()) {
                    Record record = (Record) iterator.next();
                    if (record.mEvent == AudioProfileListener.LISTEN_VIBRATE_SETTING_CHANGED) {
                        try {
                            record.mCallback.onVibrateSettingChanged(
                                vibrateType, getVibrateSetting(vibrateType));
                        } catch (RemoteException e) {
                            iterator.remove();
                            Log.e(TAG, "Dead object in notifyVibrateSettingChanged,"
                                    + " remove listener's callback: record.mBinder = "
                                    + record.mBinder + ", clients = " + mRecords.size()
                                    + ", exception = " + e);
                        }
