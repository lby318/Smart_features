...
public class PhoneWindowManager implements WindowManagerPolicy {
	...
	//starmen add begin for power key
	/* hetian 20150722 modify begin for Jira LM-162 enable power key when screen off*/
	PowerManager pm = (PowerManager) mContext.getSystemService(Context.POWER_SERVICE);
	boolean screen = pm.isScreenOn();
	if (keyCode == KeyEvent.KEYCODE_POWER && screen) {
		/* hetian 20150722 modify end for Jira LM-162 enable power key when screen off*/
		if(win != null && win.getAttrs() != null&&win.getOwningPackage().equals("com.android.factory")){
				return 0;
		}
	}
	//starmen add end for for power key
        // First we always handle the home key here, so applications
        // can never break it, although if keyguard is on, we do let
        // it handle it, because that gives us the correct 5 second
        // timeout.
        if (keyCode == KeyEvent.KEYCODE_HOME) {
			//starmen add begin for factory test home key
			/// M: [ALPS00054781]Dispatch the home key to the application @{
			if(win != null && win.getAttrs() != null&&win.getOwningPackage().equals("com.android.factory")){
					return 0;// return 0 let app handle
			}
			//starmen add end for factory test home key
    ...
    public int finishPostLayoutPolicyLw() {
        ...
        if (mKeyguardDelegate != null && mStatusBar != null) {
            if (localLOGV) Slog.v(TAG, "finishPostLayoutPolicyLw: mHideKeyguard="
                    + mHideLockScreen
                    /// M: Enable the animation log at WMS @{
                    + " mDismissKeyguard=" + mDismissKeyguard
                    + " mKeyguardDelegate.isSecure()= " + mKeyguardDelegate.isSecure());
                    /// @}
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_VOLUME_MUTE: {
			    //Moka add
				ActivityManager activityManagers = (ActivityManager)mContext.getSystemService(Context.ACTIVITY_SERVICE);
				String runningActivity=activityManagers.getRunningTasks(1).get(0).topActivity.getClassName();
				//Moka add end
				...
				//Moka add
				if(down&&runningActivity.equals("com.android.mms.ui.ClassZeroActivity")){
					TelecomManager telecomManagers = getTelecommService();
					if(telecomManagers != null)
						telecomManagers.silenceRinger();
				        result &= ~ACTION_PASS_TO_USER;
                    break;
                //Moka add
                }
            ...
            case KeyEvent.KEYCODE_POWER: {
                ...
                try{
					/* hetian 20150722 modify begin for Jira LM-162 enable power key when screen off*/
					if (activityManager.getRunningTasks(1).get(0).topActivity.getPackageName().equals("com.android.factory") && screen){
						/* hetian 20150722 modify end for Jira LM-162 enable power key when screen off*/
						result |= ACTION_PASS_TO_USER;
						return result;
	                }
				}catch(NullPointerException e){}
				/*starmen add end for nmi TV response power key in test.java 20150407*/
            ...
            //Tim add vib to home back and recent key
    ...
    private int updateSystemUiVisibilityLw() {
        ...
        /// M: BMW. If top window is a floating window,
        /// we should clear the low profile and only content flags. @{
        if (MultiWindowProxy.isSupported()
                && win.getAppToken() != null
                && MultiWindowProxy.getInstance().isFloatingByAppToken(
                    win.getAppToken().asBinder())) {
    ...
    public void dump(String prefix, PrintWriter pw, String[] args) {
        ...
        /// M: BMW.  Dump the info for mCurrentInputMethodWindow &
        /// mCurrentInputMethodTargetWindow @{
        if (MultiWindowProxy.isSupported()) {
            if (mCurrentInputMethodWindow != null) {
                pw.print(prefix); pw.print("mCurrentInputMethodWindow=");
                        pw.println(mCurrentInputMethodWindow);
            }
            if (mCurrentInputMethodTargetWindow != null) {
                pw.print(prefix); pw.print("mCurrentInputMethodTargetWindow=");
                        pw.println(mCurrentInputMethodTargetWindow);
            }
        }
        /// @}
    }
    ...
    /// M:[SmartBook] @{
    //private boolean mSmartBookPlugIn = false;
    //private IMtkHdmiManager mHDMI = null;
    //private int mSmartBookRotation = 1;
    //private static final ArrayList<String> sBlackList;
    //static {
    //    sBlackList = new ArrayList<String>();
    //    sBlackList.add("com.google.android");
    //    sBlackList.add("com.android.chrome");
    //    sBlackList.add("com.android.facelock");
    //    sBlackList.add("com.android.inputmethod.latin");
    //    sBlackList.add("com.android.vending");
   ...
   ///@}
    ...
    BroadcastReceiver mStatusBarEventReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            Log.v(TAG, "mStatusBarEventReceiver -- onReceive -- entry");

            if (action.equals(Intent.MTK_ACTION_REQUEST_TRANSIENT_STATUSBAR)) {
                if (mStatusBar != null) {
                    requestTransientBars(mStatusBar);
                }
            }
    ...
    /// M: BMW. Save the current IME window state. @{
    WindowState mCurrentInputMethodWindow = null;
    WindowState mCurrentInputMethodTargetWindow = null;

    public void setCurrentInputMethodWindowLw(WindowState ime,
            WindowState target) {
        mCurrentInputMethodWindow = ime;
