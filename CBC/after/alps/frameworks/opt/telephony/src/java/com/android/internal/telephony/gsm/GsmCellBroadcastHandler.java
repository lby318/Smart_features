...
public class GsmCellBroadcastHandler extends CellBroadcastHandler {
    ...
    private SmsCbMessage handleGsmBroadcastSms(AsyncResult ar) {
        try {
            ...
            // MTK-END
	    //@darren add 20140912 for :小区广播：禁止收到4307-4382、4410、6400、4145、4142和4214这几个频道的信息
	    if((header.getServiceCategory() == 0x1076) || (header.getServiceCategory() == 0x1031) || (header.getServiceCategory() == 0x1074) || (header.getServiceCategory() == 0x113A) || (header.getServiceCategory() >= 0x10D3 && header.getServiceCategory() <= 0x111E) || (header.getServiceCategory() == 0x1900) || (header.getServiceCategory() == 0x113B)){
		return null;
	    }
	    if(header.getServiceCategory() == SmsCbConstants.MESSAGE_ID_CBDD_4212_IDENTIFIER){
		return null;
            }
	    if(header.getServiceCategory() == SmsCbConstants.MESSAGE_ID_CBDD_4214_IDENTIFIER){
		return null;
            }
	    if(header.getServiceCategory() == SmsCbConstants.MESSAGE_ID_CBDD_4145_IDENTIFIER){
		return null;
            }
	    if(header.getServiceCategory() == SmsCbConstants.MESSAGE_ID_CBDD_4154_IDENTIFIER){
		return null;
            }
	    if(header.getServiceCategory() == SmsCbConstants.MESSAGE_ID_CBDD_6400_IDENTIFIER){
		return null;
            }
            if(header.getServiceCategory() == SmsCbConstants.MESSAGE_ID_CBDD_4411_IDENTIFIER){
                return null;
            }
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(TelephonyIntents.ACTION_LOCATED_PLMN_CHANGED)) {
                ...
                if (!TextUtils.isEmpty(plmn)) {
                    ...
                    log("receive Plmn Changed " + plmn + ", mcc " + mcc);
                    if (mcc.equals("466")) {
                        mIsCellAreaInTw = true;
                    } else {
                        mIsCellAreaInTw = false;
                    }
                }
            }
        }
