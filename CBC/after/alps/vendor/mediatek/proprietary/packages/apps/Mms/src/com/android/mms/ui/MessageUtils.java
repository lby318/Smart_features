...
//        final long totalBlocks = stat.getBlockCount();
    ...
    public static String CBCformatTimeStampStringExtend(Context context, long when, boolean empty) {
        Time then = new Time();
        then.set(when);
        Time now = new Time();
        now.setToNow();
        ...
        // If the message is from a different year, show the date and year.
            format_flags |= DateUtils.FORMAT_SHOW_TIME;
	    String yeardata = String.valueOf(then.monthDay) + "/" + String.valueOf((then.month + 1)) + "/" + String.valueOf(then.year);
            String time = DateUtils.formatDateTime(context, when, format_flags);
            String result;
            if(empty){			
	    result = time + " " + "|" + " " + yeardata;
            	}else{
	    result = time + "|" + yeardata;            	
            }
       ...
       // if(empty){
        return result;
        //}else{
       // return DateUtils.formatDateTime(context, when, format_flags) + "|" +yeardata;        	
        	//}
    }
	//Moka add for Azumi CL CBC
