...
public class CellBroadcastSettings extends TimeConsumingPreferenceActivity
    ...
    private void initPreference() {
	 ...
	 //removePreference(findPreference("button_language"));
	 getPreferenceScreen().removePreference(mLanguagePreference);
    ...
    private void updateChannelUIList() {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if((channelId==919)||(channelId==519)||(channelId==514))
            {
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
        ...
        if (position >= 3) {
	     ...
	     //Moka add for Auzmi CL CBC
            if(channel.getChannelId()== 519 || channel.getChannelId()== 919  || channel.getChannelId()== 514)
                 return;
            //Moka add for Auzmi CL CBC
