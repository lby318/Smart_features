			      ...
			      enum wpa_states state)
{
	...
	if (state == WPA_COMPLETED && wpa_s->assoc_freq &&
		os_strlen(wpa_s->ifname) == os_strlen("wlan0") &&
		!os_strcmp(wpa_s->ifname, "wlan0")) {

		u8 op_reg_class, op_channel;

		wpa_printf(MSG_DEBUG, "ifname %s freq %d.",
			wpa_s->ifname, wpa_s->assoc_freq);

		if (p2p_freq_to_channel(wpa_s->assoc_freq,
			&op_reg_class, &op_channel) == 0) {
			if (op_channel && op_reg_class && wpa_s->global->p2p) {

				struct p2p_data *p2p = wpa_s->global->p2p;
				wpa_printf(MSG_DEBUG, "P2P: Update p2p channels "
					"when legacy is connected %d/%d",
	...
	/*
	* ALPS01439965
	* User cannot wait for timeout to delete the GO from scan res list.

	* This will result in wps overlap if the ex-connected GO is still in
	* the scan list. So, flush the res list when supplicant state goes
	* DISCONNECTED. GC will re-scan when do next p2p connect.
	*/
	if ((state == WPA_DISCONNECTED) &&
		(os_strlen(wpa_s->ifname) == os_strlen("p2p0")) &&
		(os_strcmp(wpa_s->ifname, "p2p0") == 0)) {
		wpa_printf(MSG_DEBUG, "P2P: Flush the wpa_s->bss list");
		wpa_bss_flush(wpa_s);
	}
	if (state == WPA_COMPLETED && wpa_s->assoc_freq &&
		os_strlen(wpa_s->ifname) == os_strlen("wlan0") &&
		...
		if (p2p_freq_to_channel(wpa_s->assoc_freq,
			...
			if (op_channel && op_reg_class && wpa_s->global->p2p) {
				...
				struct p2p_data *p2p = wpa_s->global->p2p;
				wpa_printf(MSG_DEBUG, "P2P: Update p2p op channel "
					"when legacy is connected %d/%d",
					op_reg_class, op_channel);
					 ...
					 struct wpa_ie_data *ie)
{
	...
	if (ssid->proto & WPA_PROTO_WAPI) {
			wpa_msg(wpa_s, MSG_INFO, "[WAPI-Debug]WPA: No support "
				"wpa_supplicant_suites_from_ai"
				" for WAPI");
			return -1;
	}
#endif
...
int wpa_gen_wapi_ie(struct wpa_supplicant *wpa_s, u8 *wapi_ie)
{
	...
	*pos++ = 0;
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WAPI_CERT)
		os_memcpy(pos, WAPI_AUTH_KEY_MGMT_WAI, WAPI_SELECTOR_LEN);
	else if (wpa_s->key_mgmt == WPA_KEY_MGMT_WAPI_PSK)
	...
	struct wpa_ssid *ssid)
{
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WAPI_PSK) {
		...
		if (ssid->passphrase) {
			...
			wpa_hexdump(MSG_MSGDUMP,"[WAPI-Debug][wapi-passphrase]",
					(const u8*) ssid->passphrase,
					strlen((char*)ssid->passphrase));
			if (!ssid->passphrase) {
				wpa_printf(MSG_ERROR, "[WAPI-Debug] ssid->passphrase is NULL,"
					" Fatal error %s ",__FUNCTION__);
				return -1;
			}
			...
			wpa_hexdump(MSG_MSGDUMP,"[WAPI-Debug][wapi-hex]",
					wapi_param.para.kv, wapi_param.para.kl);
		}
		/*
		* wpa_printf(MSG_DEBUG, "[WAPI-Debug!!!!!*_*!!]
		* auth %d type %d length %d %s",
...
#define KEYSTORE_SCHEME_LEN		11
		...
		if (ssid && ssid->eap.ca_cert2)
			...
			wpa_printf(MSG_ERROR, "[WAPI-Debug]ca_cert2 = NULL\n");
			return -1;
		}
		...
		value_length = keystore_get((const char*)&cert_uri[KEYSTORE_SCHEME_LEN],
					user_cert_idx_len-KEYSTORE_SCHEME_LEN,
					(u8**)&cert_value);
		/*
		* from android 4.3, if keystroe encountered some error, the return
		* value of keystore_get will be 0
		...
		*/
		...
		/*
		* wpa_hexdump(MSG_DEBUG, "user_cert from supplicant=>",
		* (const u8 *) cert_value, 512);
		*/
		...
		/*
		* memory is allocate in keystore_get, so we should free here
		*/
		os_free(cert_value);
		...
		value_length = keystore_get((const char*)&cert_uri[KEYSTORE_SCHEME_LEN],
				as_cert_idx_len-KEYSTORE_SCHEME_LEN,
				(u8**)&cert_value);
		if (value_length <= 0) {
			wpa_msg_ctrl(wpa_s, MSG_INFO, WPA_EVENT_EAP_NO_CERTIFICATION);
		...
		/*
		* memory is allocate in keystore_get,
		* so we should free here
		*/
		os_free(cert_value);
			      ...
			      u8 *wpa_ie, size_t *wpa_ie_len)
{
	...
	if (proto != WPA_PROTO_WAPI) {
		...
		if (wpa_key_mgmt_wpa_psk(ssid->key_mgmt) /* ssid->key_mgmt & WPA_KEY_MGMT_PSK */)
		...
		*wpa_ie_len = wpa_gen_wapi_ie(wpa_s, wpa_ie);
		...
		/*
		* should check ie len to avoid NE when dump IEs
		*/
		if (*wpa_ie_len <= 0) {
	...
	if (bss && (wpa_bss_get_vendor_ie(bss, WPA_IE_VENDOR_TYPE) ||
		...
		if (wpa_supplicant_set_suites(wpa_s, NULL, ssid,
			...
			wpa_printf(MSG_WARNING, "WPA: Failed to set WPA key "
				"management and encryption suites (no scan results)");
				wpas_connect_work_done(wpa_s);
			return;
		}
#endif
...
#if defined(CONFIG_MTK_SCC) || defined(CONFIG_MTK_WFD_LEGACY_COEX)
		else
			params.freq.freq = bss->freq;
#endif
...
/* MTK_OP01_PROTECT_END */
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WPA_NONE) {
		...
		if (wpa_s->key_mgmt &
			...
			wpa_printf(MSG_DEBUG,
				"[WAPI-Debug] set WAPI Auth Time in 35 secs"
				   ...
				   struct wpa_ssid *ssid)
{
	...
	wpa_s->reassociate = 1;
#ifdef CONFIG_ALWAYS_SPECIFIC_SCAN
	/*
	* reset prev_scan_ssid to make specified
	* scan start with the first SSID.
	* so this can make all SSIDs probe request individually
	*/
	wpa_s->prev_scan_ssid = WILDCARD_SSID_SCAN;
#endif
#ifdef CONFIG_MTK_WFD_LEGACY_COEX
	if (wpa_s->connect_without_scan ||
	    wpa_supplicant_fast_associate(wpa_s) != 1) {
		/*
		* ALPS01841137: [Side Effect][K2 64][LMP][TK][WiFi]
...
extern int stage_scan_occur;
extern void wpa_supplicant_req_stage_scan(struct wpa_supplicant *wpa_s, int sec, int usec);
#endif
...
int wpa_supplicant_driver_init(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->l2_wai &&
		l2_packet_get_own_addr(wpa_s->l2_wai, wpa_s->own_addr)) {
		wpa_printf(MSG_ERROR,
			"[WAPI !!>_<!!] Failed to get own L2 address for WAPI");
		return -1;
	}
#endif
	...
	if (wpa_supplicant_enabled_networks(wpa_s)) {
		...
		if (!wpa_s->p2p_mgmt &&
		    wpa_supplicant_delayed_sched_scan(wpa_s,
						      ...
						      100000))
#ifdef CONFIG_MTK_STAGE_SCAN
			wpa_printf(MSG_DEBUG, "hw.num_modes = %d", wpa_s->hw.num_modes);
			if ((wpa_s->hw.num_modes == 3) && (stage_scan_occur == 0)) {
				stage_scan_occur = 1;
				wpa_printf(MSG_DEBUG, "[STAGE_SCAN] Start stage_scan");
				wpa_supplicant_req_stage_scan(wpa_s, interface_count % 3,
							100000);
			} else
...
#if defined (CONFIG_MTK_SCC) && defined (CONFIG_MTK_WFD_LEGACY_COEX)
		/*
		* MTK solution has two phys for legacy & p2p
		* must add wpa_s to the same radio list if want
		* SCC work
		*/
		if (radio && os_strcmp(rn, radio->name)) {
...
#ifdef CONFIG_MTK_PRESET_SSID
		if (wpa_s->conf->overlay_loaded)
			wpa_s->confanother = NULL;
		else
#endif
					      ...
					      const char *value)
{
	...
	case WPA_CTRL_REQ_EAP_IDENTITY:
		...
		if (ssid == wpa_s->current_ssid && wpa_s->wpa_state < WPA_ASSOCIATING)
			wpa_s->reassociate = 1;
		break;
